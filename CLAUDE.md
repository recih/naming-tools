...
- **`src/routes/__root.tsx`**: Root layout with HTML document structure, header, navigation, and detail panel
  - Uses `shellComponent: RootDocument` for proper SSR/CSR hybrid rendering
  - Imports CSS with `?url` suffix and injects via `head()` function
  - Includes `<HeadContent />` and `<Scripts />` components for proper asset injection
...
**Important**: The root route uses a `shellComponent` to define the complete HTML document structure. This is critical for CSS and script injection in TanStack Start.
...
│   │   ├── __root.tsx              # Root layout with shellComponent
...
- **SSR/CSR hybrid**: Implemented SSR-safe code with proper browser API checks
- **Cloudflare Workers**: Added deployment support with conditional plugin loading
- **New dependencies**: TanStack Router, TanStack Start, @cloudflare/vite-plugin, Wrangler
- **CSS loading fix**: Implemented shellComponent with HeadContent and Scripts for proper asset injection
- **Bug fixes**:
  - Fixed infinite HMR loop by reordering Vite plugins
  - Fixed blank page error by disabling Cloudflare plugin in development
